...
def test_crop_center():
	...
	#################################### test with 4 elements in centeR0_rect #########################################
	print('test 2d matrix intersected on the top left')
	np_data = (np.random.rand(5, 5) * 255.).astype('uint8')
	centeR0_rect = [1, 2, 4, 6]
	img_padded, crop_bbox, crop_bbox_clipped = image_crop_center(np_data, centeR0_rect=centeR0_rect, pad_value=10)
	print(np_data)
	print(img_padded)
	...
	centeR0_rect = [0, 50, 100, 100]
	img_cropped, crop_bbox, crop_bbox_clipped = image_crop_center(img, centeR0_rect=centeR0_rect, pad_value=100)
	...
	#################################### test with 2 elements in centeR0_rect #########################################
	...
	centeR0_rect = [100, 100]
	img_cropped, crop_bbox, crop_bbox_clipped = image_crop_center(img, centeR0_rect=centeR0_rect, pad_value=100)
	visualize_image(img, vis=True)
	visualize_image(img_cropped, vis=True)

	print('test with color image, interesected')
	centeR0_rect = [600, 600]
	img_cropped, crop_bbox, crop_bbox_clipped = image_crop_center(img, centeR0_rect=centeR0_rect, pad_value=100)
	visualize_image(img, vis=True)
	visualize_image(img_cropped, vis=True)
	...
	centeR0_rect = [100, 100]
	img_cropped, crop_bbox, crop_bbox_clipped = image_crop_center(img.convert('L'), centeR0_rect=centeR0_rect, pad_value=100)
